...
int8_t init_comm(meter_comm_params_t *meter_comm_params)
{
	...
	sprintf(debug_file_name,"%s/dlms_api_meter_id_%d.log",LOG_DIR,meter_comm_params->meter_id);
	
	memset(g_ls_data_dir_path,0,sizeof(debug_file_name));
...
{
	if(last_num_days_read>30)
	{
		printf("Received More number of block to be read. Max is 30\n");
		return -1;
	}
	else
	{
