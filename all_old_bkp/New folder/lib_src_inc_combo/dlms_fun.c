...
int32_t save_ls_data_file(void)
{
		...
		(g_block_val_info.date_time.minute==0))
	{
		...
		if(g_meter_mfg_type==LNT_METER_MFG_TYPE)
		{
			...
			g_block_val_info.date_time.hour=23;
			g_block_val_info.date_time.minute=59;

			memset(curr_ls_file_name,0,sizeof(curr_ls_file_name));
			sprintf(curr_ls_file_name,"%02d_%02d_%04d",
								time_stamp.tm_mday,
								time_stamp.tm_mon+1,
								time_stamp.tm_year+1900);
				
			dbg_log(REPORT,"%-20s : After -1 day Curr file_name : %s\n",fun_name,curr_ls_file_name);		
		}
