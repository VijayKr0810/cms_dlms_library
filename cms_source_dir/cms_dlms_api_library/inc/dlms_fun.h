...
#define EVENT_ENTRY_ORDER_INT_CLASS		0X0007
...
#define EVENT_ENTRY_ORDER_ATTR_NO 		0X0005
...
typedef struct
{
	...
	uint8_t			value[16];
	uint8_t			obis_code[6];
}param_dec_obis_val_info_t;

typedef struct
{
	uint8_t						tot_num_params;
	param_dec_obis_val_info_t 	param_obis_val_info[128];
}
all_param_obis_val_info_t;
...
int32_t get_dp_obis_code_det(meter_comm_params_t *meter_comm_params,gen_params_det_t *recv_gen_inst_param_det);
