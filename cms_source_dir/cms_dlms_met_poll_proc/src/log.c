...
int32_t met_poll_dbg_log(uint8_t mode, const char *p_format, ...)
{
	...
	//sprintf(file_name,"%s",poll_debug_file_name);
	sprintf(file_name,"%s/%s_%d.log","/usr/cms/log","cms_met_poll_proc",g_port_idx);
